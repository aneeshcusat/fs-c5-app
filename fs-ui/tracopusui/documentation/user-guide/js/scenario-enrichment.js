...
(function (global) {
  ...
  "hrms-submit-timesheet": {
    "context": "Employees submit timesheet hours for manager approval when timesheetApprovalEnabled is active. Default (partialWeekSubmissionEnabled off): submit the completed week as one unit and the week locks while pending. When partialWeekSubmissionEnabled is on: select day chips and submit day batches (activityDates); leftover DRAFT days stay editable; each batch is a separate approval request. Resolve validation conflicts before submit.",
    "summary": "Submit a timesheet week or day batch into the approval workflow.",
    "permissions": [
      {
        "name": "Time entry",
        "path": "timeSheet.enableTimeSheetEntry",
        "level": "Complete",
        "notes": "Full week when partial off; selected days when partial on."
      },
      {
    ...
    "featureFlags": [
      {
        "flag": "timesheetApprovalEnabled",
        "group": "timesheetAdditional",
        "whenEnabled": "Submit for approval action available.",
        "whenDisabled": "No submission step; hours remain draft/saved only."
      },
      {
        "flag": "partialWeekSubmissionEnabled",
        "group": "timesheetAdditional",
        "whenEnabled": "Day chips + multiple submission batches per week; leftover DRAFT days editable while other days pending/approved.",
        "whenDisabled": "Full-week submit only; week locked while PENDING; no submission table."
      },
      {
        "flag": "universalApprovalInboxEnabled",
        "group": "tracopusWorkspaceAdditional",
        "whenEnabled": "Manager sees WEEK: or SUBMISSION: item in Workspace → Approvals.",
    ...
    "behaviorChanges": [
      {
        "role": "Individual contributor",
        "effect": "Partial off: submit locks the week. Partial on: only submitted days lock; remaining DRAFT days stay editable.",
        "notes": "Status chips show Submitted / day badges."
      },
      {
        "role": "Manager",
        "effect": "Receives notification per week (flag off) or per submission batch (flag on).",
    ...
    "considerations": [
      ...
      "Keep partialWeekSubmissionEnabled off unless the org needs mid-week batches.",
      "Communicate weekly submit deadline to all ICs.",
      "Test rejection and resubmit loop end-to-end (week and batch).",
      "Verify delegate receives notification when primary away."
    ],
    "verify": [
      "Submit disabled until validation passes.",
      "Flag off: status Submitted for whole week; employee cannot edit while Submitted.",
      "Flag on: day chips visible; leftover DRAFT days editable while a batch is pending.",
      "Manager sees item in pending approvals (WEEK or SUBMISSION).",
      "Rejection returns editable days with comment visible.",
      "Approved week/batch shows Approved status chip."
    ],
    "screenDoc": "hrms/timesheet.html"
  },
  "hrms-approve-timesheet": {
    ...
    "featureFlags": [
      ...
      },
      {
        "flag": "partialWeekSubmissionEnabled",
        "group": "timesheetAdditional",
        "whenEnabled": "Pending cards are submission batches; multi-pending reject/approve requires submissionId; reopen may target a batch.",
        "whenDisabled": "One pending week card; week-level approve/reject/reopen."
      },
      {
        "flag": "approvalReminderEnabled",
        "group": "approvalDelegation",
        "whenEnabled": "SLA reminder emails for stale pending timesheets (counts submissions when partial on).",
    ...
    "behaviorChanges": [
      {
        "role": "Manager",
        "effect": "Approve/reject direct report weeks or batches in team scope.",
    ...
    "verify": [
      "Pending panel lists submitted weeks or submission batches in scope.",
  ...
  "hrms-apply-leave": {
    "context": "Employees apply for leave through People → Leave management (`/people/leaves`) when leaveManagementEnabled is active. Balances must already exist from Admin → Employee leave config (OB import / Adjust balance) or accrual. Day count uses working-week rules and People holidays. Approved leave may promote to timesheet leave rows and calendar blocks.",
    "summary": "Submit a leave request with balance and overlap validation.",
    "permissions": [
      {
        "name": "Leave menu",
        "path": "peopleModules.leavesEnabled",
        "level": "Active",
        "notes": "Requires leaveManagementEnabled org flag. Legacy hrmsModules.leavesEnabled still accepted in some roles."
      },
      {
        "name": "Create leave",
  ...
  "admin-configure-global-leave": {
    "context": "HR configures enterprise leave in Global Policy Center: jurisdictions, templates, versioned policies, holiday calendar keys, then publishes. Opening balances are set on Admin → Employee leave config (OB template + dry-run import, or Adjust balance) — publish does not invent balances. Detailed supported types / CFA map: Leave Policies User Guide (tabs). See Leave Configuration screen doc for the go-live checklist.",
  ...
  "admin-employee-leave-config": {
    "context": "HR sets per-employee opening balances and adjustments on Admin → Employee leave config after policies are published. For cutover, download the OB template CSV from the same page, fill from ZingHR, then dry-run import before live write.",
    "summary": "Adjust balances one-by-one or download the opening-balance CSV template for bulk cutover.",
    "permissions": [
      {
        "name": "Leave policy admin",
        "path": "adminModules.leavePoliciesEnabled",
        "level": "Admin",
        "notes": "Requires policyAdminEnabled on leave access control."
      }
    ],
    "featureFlags": [
      {
        "flag": "leaveManagementEnabled",
        "group": "leaveAdditional",
        "whenEnabled": "Balances used by apply/submit; page workflows active.",
        "whenDisabled": "Native leave module banner; adjustments blocked."
      }
    ],
    "behaviorChanges": [
      "Download OB template builds blank entitlementAmount/amount rows (PL/CL/SL by location).",
      "Single-employee Adjust balance can credit opening entitlement and/or +/- available days.",
      "Adjustments of 5+ days require a second confirmer employee id.",
      "Bulk import uses LeaveBalanceEngine via POST /api/v2/admin/data-import/leave-balance (dryRun supported).",
      "Accrual / carry-forward / COMP_OFF_EXPIRE schedules continue after opening balances."
    ],
    "verify": [
      "OB template downloads from Download OB template / Download CSV.",
      "Dry-run import returns validated rows with no balance writes.",
      "Live import (dryRun=false) updates balance cards.",
      "Employee can apply leave against remaining balance on People → Leaves."
    ],
    "warnings": [
      "Do not invent opening balances without an HR / ZingHR export.",
      "Template only covers loaded team employees — select the correct team filter first."
    ],
    "screenDoc": "hrms/leave-configuration.html#employee-balances"
  },
  "admin-leave-ob-bulk-import": {
    "context": "Cutover path for importing ZingHR / legacy leave balances into canonical Employee + LeaveType + LeaveBalanceEngine. UI template download lives on Employee leave config; API supports dryRun=true before live write.",
    "summary": "Download template → fill → dry-run → live import opening balances.",
    "permissions": [
      {
        "name": "Leave policy admin",
        "path": "adminModules.leavePoliciesEnabled",
        "level": "Admin",
        "notes": "Same page as employee leave config."
      }
    ],
    "featureFlags": [
      {
        "flag": "leaveManagementEnabled",
        "group": "leaveAdditional",
        "whenEnabled": "Import API accepts rows for the team.",
        "whenDisabled": "Import returns Forbidden."
      }
    ],
    "behaviorChanges": [
      "dryRun=true (query or body) validates only — no LeaveBalanceEngine writes.",
      "Response includes reportCsv and per-row validated/imported/failed status.",
      "In-batch duplicate employee+leave type rows are rejected."
    ],
    "verify": [
      "Dry-run shows validated for mapped employees.",
      "Live import increases balance/entitlement on Employee leave config cards.",
      "Sample apply on Leave Management succeeds against imported balance."
    ],
    "warnings": [
      "Never POST dryRun=false until dry-run is clean.",
      "Map ZingHR emp codes to Tracopus employeeId/empCode before fill."
    ],
    "screenDoc": "hrms/leave-configuration.html#employee-balances"
  },
  ...
  "admin-role-permissions": {
    ...
    "screenDoc": "hrms/roles-permissions.html"
  },
